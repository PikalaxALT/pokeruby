...
static bool8 LoadAppropiateBankSprite(u8 bank)
{
    if (bank < gBattlersCount)
    {
        ...
        {
            if (!eBattlerSpritesData[bank].substituteSprite)
        ...
        else if (!eBattlerSpritesData[bank].substituteSprite)
...
static void sub_807B184(u8 bank)
{
    if (bank < gBattlersCount)
    {
        ...
        gSprites[gBattlerSpriteIds[bank]].invisible = eBattlerSpritesData[bank].invisible;
    }
}
