...
bool8 move_anim_start_t3(u8 a, u8 b, u8 c, u8 d, u16 e)
{
    ...
    if (eBattlerSpritesData[a].substituteSprite && sub_803163C(d) == 0)
        return TRUE;
    if (eBattlerSpritesData[a].substituteSprite && d == 2 && gSprites[gBattlerSpriteIds[a]].invisible)
    {
...
void BattleLoadOpponentMonSprite(struct Pokemon *pkmn, u8 b)
{
    ...
    if (eBattlerSpritesData[b].transformedSpecies == 0)
    {
    ...
    {
        species = eBattlerSpritesData[b].transformedSpecies;
...
{
    sub_8031EE8();
    memset(eBattlerSpritesData, 0, 0x10);
}
...
void sub_8031F24(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
        eBattlerSpritesData[i].invisible = gSprites[gBattlerSpriteIds[i]].invisible;
}

void sub_8031F88(u8 a)
{
    eBattlerSpritesData[a].invisible = gSprites[gBattlerSpriteIds[a]].invisible;
}
...
void sub_8031FC4(u8 a, u8 b, bool8 c)
{
    ...
    if (c)
    {
        ...
        if (eBattlerSpritesData[a].transformedSpecies != 0)
        {
            BlendPalette(paletteOffset, 16, 6, RGB(31, 31, 31));
            CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
        }
    ...
    else
    {
        ...
        if (!IsContest())
        {
            eBattlerSpritesData[a].transformedSpecies = species;
...
{
    if (b == 0xA4)
        eBattlerSpritesData[a].substituteSprite = 1;
}

void sub_80324E0(u8 a)
{
    eBattlerSpritesData[a].substituteSprite = 0;
}
...
void HandleLowHpMusicChange(struct Pokemon *pkmn, u8 b)
{
    ...
    {
        if (!eBattlerSpritesData[b].unk0_1)
        {
            if (!eBattlerSpritesData[b ^ 2].unk0_1)
                PlaySE(SE_LOW_HEALTH);
            eBattlerSpritesData[b].unk0_1 = 1;
        }
    }
    else
    {
        eBattlerSpritesData[b].unk0_1 = 0;
        if (!IsDoubleBattle())
        {
            m4aSongNumStop(SE_LOW_HEALTH);
            return;
        }
        if (IsDoubleBattle() && !eBattlerSpritesData[b ^ 2].unk0_1)
        {
            m4aSongNumStop(SE_LOW_HEALTH);
            return;
        }
...
void BattleStopLowHpSound(void)
{
    ...
    eBattlerSpritesData[r4].unk0_1 = 0;
    if (IsDoubleBattle())
        eBattlerSpritesData[r4 ^ 2].unk0_1 = 0;
    m4aSongNumStop(SE_LOW_HEALTH);
}
...
void sub_80328A4(struct Sprite *sprite)
{
    ...
    if (gAnimScriptActive || r7->invisible)
        invisible = TRUE;
    else if (eBattlerSpritesData[r4].transformedSpecies != 0 && gEnemyMonElevation[eBattlerSpritesData[r4].transformedSpecies] == 0)
        invisible = TRUE;
    if (eBattlerSpritesData[r4].substituteSprite)
        invisible = TRUE;
...
void sub_8032984(u8 a, u16 b)
{
    ...
    {
        if (eBattlerSpritesData[a].transformedSpecies != 0)
            b = eBattlerSpritesData[a].transformedSpecies;
...
{
    eBattlerSpritesData[a].transformedSpecies = 0;
    gBattleMonForms[a] = 0;
    if (b == 0)
        sub_80324E0(a);
}
